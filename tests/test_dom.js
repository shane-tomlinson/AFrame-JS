function DOMTest( adapter, name ) {
    ...
    return {
        ...
        testClosest: function() {
            var closest = DOM.closest( "div", ".list" );
            Assert.areSame( "AFrame_List", $( closest ).attr( "id" ), "correct ancestor found" );
        },
        ...
        testHasAttrElementNonExistentElement: function() {
            Assert.isFalse( DOM.hasAttr( "#nonexistent", "data-attr" ), "false returned for non-existent element" );
        },

        testRemoveAttr: function() {
            DOM.setAttr( "#testSetInnerNonInput", "data-attr", "test attr" );

            DOM.removeAttr( "#testSetInnerNonInput", "data-attr");
            var val = jQuery( "#testSetInnerNonInput" ).attr( "data-attr" );
            Assert.areSame( "undefined", typeof val, "removeAttr removes attribute" );
        },
        ...
        testInsertAfter: function() {
            ...
            DOM.insertAfter( DOM.createElement( "div", "second div" ), first );

            Assert.areSame( 2, jQuery( ".DOMSelection" ).children().length, "insertAfter inserts after" );
        },
        ...
        testInsertBefore: function() {
            ...
            Assert.areSame( 2, jQuery( ".DOMSelection" ).children().length, "insertBefore inserts" );
        },
        ...
        testShowHide: function() {
            DOM.show( "#hiddenElement" );
            Assert.areSame( true, jQuery( "#hiddenElement" ).is( ":visible" ) );

            DOM.hide( "#hiddenElement" );
            Assert.areSame( false, jQuery( "#hiddenElement" ).is( ":visible" ) );
        },

        testSlideDownUp: function() {
            DOM.slideDown( "#hiddenElement" );
            Assert.areSame( true, jQuery( "#hiddenElement" ).is( ":visible" ) );

            DOM.slideUp( "#hiddenElement", 0 );
            // this requires an asynchronous test and right now I am not sure
            // how to do this with a QUnit made to look like YUI test
            //Assert.areSame( false, jQuery( "#hiddenElement" ).is( ":visible" ) );
        },

        testFadeInOut: function() {
            DOM.fadeIn( "#hiddenElement" );
            Assert.areSame( true, jQuery( "#hiddenElement" ).is( ":visible" ) );

            // this requires an asynchronous test and right now I am not sure
            // how to do this with a QUnit made to look like YUI test
            DOM.fadeOut( "#hiddenElement", 0 );
            //Assert.areSame( false, jQuery( "#hiddenElement" ).is( ":visible" ) );
